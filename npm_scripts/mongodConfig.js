...
const pathMongodDataDb = dir.concat("mongodb\\data\\db"),
...
const creatingChild = execSync(
    `powershell -command \"start-process  \'C:\\Program Files\\MongoDB\\Server\\4.0\\bin\\mongod.exe\' \'-f ${pathMongoCfg} --install\' -verb runAs\"`,
    (error, stdout, stderr) => {
        if (error) {
...
// remove service from cmd: powershell -command "start-process  'C:\Program Files\MongoDB\Server\4.0\bin\mongod.exe' '-f C:\Users\AndreyRi\WebstormProjects\portalForStudents\mongodb\mongod.cfg --remove' -verb runAs"

// starting mongod service
const startingChild = execSync(
    "powershell -command \"start-process 'net' 'start MongoDBPortalForStudents' -verb runAs\"",
    (error, stdout, stderr) => {
        if (error) {
