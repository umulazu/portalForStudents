...
const WeekRow = ({ workweek, weekRowClasses, dayRowClasses }) => {
    ...
    return (
        <React.Fragment>
            <tr className={weekRowClasses} onClick={handleWeekClick} data-test-component="WeekClick">
                <th colSpan="6" className={firstChildClasses}>
                    {workweek.numberOfTheWeek}-я неделя
                </th>
            </tr>
            {isCollapsed
                ? null
                : workweek.daysOfTheWeek.map(workday => (
                      <DayRow
                          workday={workday}
