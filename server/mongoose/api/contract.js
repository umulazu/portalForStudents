...
export const getHoursPerDay = async studentId => {
    const contract = await Contract.findOne(
        { student: studentId, status: "active" },
        "hoursPerWeek"
    );
    return contract ? contract.hoursPerWeek / 5 : 0;
};
...
export const updateWorkday = async (contractId, workdayId, workday) => {
    await Contract.updateOne(
        { _id: contractId, "workdays._id": workdayId },
        { $set: { "workdays.$": workday } }
    ).exec();
    return workdayId;
};
...
export const getWorkdayIdByDate = async (contractId, date) => {
    ...
    const workdays = await contract.workdays.filter(workday =>
        areEqualDates(workday.date, date)
    );
...
export const getWorkdayById = async (contractId, workdayId) => {
    const contract = await Contract.findOne({ _id: contractId }).exec();
    const workdays = await contract.workdays.filter(
        workday => workday._id.toString() === workdayId
    );
...
export const addTime = async (contractId, workdayId, time) => {
    ...
    await Contract.updateOne(
        { _id: contractId, "workdays._id": workdayId },
        {
            $set: { "workdays.$.timeWorked": diff + workday.timeWorked },
            $push: { "workdays.$.time": time },
        }
    ).exec();
